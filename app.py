...
# Forzar reconocimiento de videos
...
# Soporte para formularios con mucho texto
...
def guardar_datos():
    data = {"usuarios": usuarios_db, "foros": foro_data, "chats": chats_privados}
...
def enviar_codigo(correo_destino, codigo):
    ...
    msg['Subject'] = f"🤖 CÓDIGO DE ACCESO: {codigo}"
    msg.attach(MIMEText(f"Tu código para activar tu cuenta es: {codigo}", 'plain'))
    try:
        # CAMBIO: Usamos SMTP_SSL y puerto 465 para mayor compatibilidad
...
def comentar(categoria, post_id):
    if 'user' not in session: return redirect('/')
    txt = (request.form.get('comentario') or "").strip()
    if txt:
        for p in foro_data.get(categoria, []):
            if p['id'] == post_id:
                p['comentarios'].append({"autor": session['user'], "texto": txt, "fecha": datetime.now().strftime("%H:%M")})
        guardar_datos()
    return redirect(url_for('ver_foro', categoria=categoria))
...
def mis_chats():
    ...
    for sala_id in chats_privados:
        if me in sala_id.split('_'):
            otro = sala_id.replace(me, "").replace("_", "")
...
def chat_privado(amigo):
    ...
    if request.method == 'POST':
        ...
        if archivo and archivo.filename != '':
            fname = secure_filename(archivo.filename)
            ext = os.path.splitext(fname)[1].lower()
            ftype = 'img' if ext in ['.png', '.jpg', '.jpeg', '.gif', '.webp'] else 'vid' if ext in ['.mp4', '.mov', '.avi', '.webm'] else 'file'
            archivo.save(os.path.join(app.config['UPLOAD_FOLDER'], fname))
        if msg or fname:
...
if __name__ == '__main__':
    # CAMBIO: Puerto dinámico para despliegue
